...
expName = 'v5'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/mac/Desktop/Lab in Psych/V5/v5_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_4 = visual.TextStim(win=win, name='text_4',
        text='The experiment contains 2 phases. \n\nIn the first phase; a word will be shown with a scale ranging from "familiar" to "unfamiliar", and you have to click on the scale how familiar or unfamiliar the word is to you. \n\nThe second phase will involve you typing words or filling in the blanks for some words that have letters missing, type the full word and click submit\n\nPress SPACE to start',
        font='Arial',
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Phase1.started', globalClock.getTime(format='float'))
    # keep track of which components have finished
    Phase1Components = [text_5]
    ...
    for thisComponent in Phase1Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Phase1.stopped', globalClock.getTime(format='float'))
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('studyword.csv'),
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('testword.csv'),
    ...
    for thisTrial_2 in trials_2:
        ...
        # keep track of which components have finished
        testComponents = [testword, textbox, submit_text, mouse]
        ...
        while continueRoutine:
            ...
            if testword.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if testword is stopping this frame...
            if testword.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > testword.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
                    testword.tStop = t  # not accounting for scr refresh
                    testword.tStopRefresh = tThisFlipGlobal  # on global time
                    testword.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'testword.stopped')
                    # update status
                    testword.status = FINISHED
            ...
            if mouse.status == STARTED:  # only update if started and not finished!
                ...
                if buttons != prevButtonState:  # button state changed?
                    ...
                    if sum(buttons) > 0:  # state changed to a new click
                        ...
                        if gotValidClick:
                            _corr = 0
                            _corrAns = environmenttools.getFromNames(study_word, namespace=locals())
                            for obj in _corrAns:
                                # is this object clicked on?
                                if obj.contains(mouse):
                                    _corr = 1
    ...
    while continueRoutine and routineTimer.getTime() < 0.5:
        ...
        if thank.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > thank.tStartRefresh + 0.5-frameTolerance:
                # keep track of stop time/frame for later
