...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/mac/Desktop/Lab in Psych/V6/v6_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "BeepSound" ---
    Sound_1 = sound.Sound(
        'A', 
        secs=0.3, 
        stereo=True, 
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='Report the delay between the key press and the beep sound (0-999). \nClick SUBMIT after entering the value',
        font='Arial',
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "BeepSound" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('BeepSound.started', globalClock.getTime(format='float'))
        Sound_1.setSound(freq, secs=0.3, hamming=True)
        Sound_1.setVolume(1.0, log=False)
        Sound_1.seek(0)
        # keep track of which components have finished
        BeepSoundComponents = [Sound_1]
        for thisComponent in BeepSoundComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "BeepSound" ---
        for thisComponent in BeepSoundComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('BeepSound.stopped', globalClock.getTime(format='float'))
        Sound_1.pause()  # ensure sound has stopped at end of Routine
        # the Routine "BeepSound" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        button.reset()
        # keep track of which components have finished
        ResponsesComponents = [text_2, textbox_2, button]
