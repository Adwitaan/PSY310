...
expName = 'soloproject'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/mac/Desktop/0_Solo/soloproject_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text='PHASE 1\n\nYou will be shown a combination of black-and-white and coloured pictures, for different durations kindly observe them. \n\nPress SPACE to start',
        font='Arial',
    ...
    # --- Initialize components for Routine "original" ---
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text="PHASE 2\n\nIn this phase, you must press the 'y' key for yes - if you see a picture you have seen in the previous phase, and press the 'n' key for no if you have not. \n\nPress SPACE to start",
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    text_3 = visual.TextStim(win=win, name='text_3',
        text='Thank you for your participation! Your response has been recorded. ',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('origialconditions.csv'),
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "original" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('original.started', globalClock.getTime(format='float'))
        image1.setImage(image)
        # keep track of which components have finished
        originalComponents = [fixation1, image1]
        for thisComponent in originalComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            if fixation1.status == STARTED:
                ...
                if tThisFlipGlobal > fixation1.tStartRefresh + 7.0-frameTolerance:
            ...
            if image1.status == NOT_STARTED and tThisFlip >= 7.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if image1.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image1.tStartRefresh + duration-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        # --- Ending Routine "original" ---
        for thisComponent in originalComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('original.stopped', globalClock.getTime(format='float'))
        # the Routine "original" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
    ...
    _keyresp_allKeys = []
    # keep track of which components have finished
    instructions2Components = [text_2, keyresp]
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('mixedconditions.csv'),
    ...
    for thisTrial_2 in trials_2:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('mixed.started', globalClock.getTime(format='float'))
        ...
        _key_resp_allKeys = []
        # keep track of which components have finished
        mixedComponents = [fixation2, image_2, key_resp]
        for thisComponent in mixedComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            if fixation2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixation2.tStartRefresh + 7.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if image_2.status == NOT_STARTED and tThisFlip >= 7.0-frameTolerance:
                # keep track of start time/frame for later
        ...
        for thisComponent in mixedComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('mixed.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "mixed" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
